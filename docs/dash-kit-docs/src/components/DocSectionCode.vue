<template>
    <div v-if="!embedded" class="relative doc-section-code">
        <div class="flex surface-card align-items-center justify-content-end rounded absolute z-1"
            ...
            <template v-if="codeMode !== 'basic' && !hideToggleCode">
                <Button
                    ...
                    label="Composition API" @click="codeLang = 'composition'"></Button>
                <Button
                    ...
                    label="Options API" @click="codeLang = 'options'"></Button>
            </template>

            <template v-if="!hideToggleCode">
                <Button v-tooltip.bottom="{ value: 'Toggle Full Code', class: 'doc-section-code-tooltip' }" type="button"
                    ...
                    icon="pi pi-code"></Button>
            </template>

            <template v-if="!hideToggleCode && code.data">
                <Button v-tooltip.bottom="{ value: 'View Code', class: 'doc-section-code-tooltip' }" type="button"
                    @click="onToggleData"
                    class="p-button-rounded p-button-text p-button-plain h-2rem w-2rem p-0 inline-flex align-items-center justify-content-center"
                    icon="pi pi-database">
                </Button>
            ...
            <template v-if="!hideCodeSandbox">
                <Button v-tooltip.bottom="{ value: 'Edit in CodeSandbox', class: 'doc-section-code-tooltip' }" type="button"
                    class="p-button-rounded p-button-text p-button-plain h-2rem w-2rem p-0 inline-flex align-items-center justify-content-center"
                    @click="showCodesandbox">
                    <template #icon="slotProps">
                        <svg role="img" :class="slotProps.class" viewBox="0 0 24 24" width="16" height="16"
                            fill="var(--text-color-secondary)" style="display: 'block'">
                            <path
            ...
            <template v-if="!hideStackBlitz">
                <Button v-tooltip.bottom="{ value: 'Edit in StackBlitz', class: 'doc-section-code-tooltip' }" type="button"
                    class="p-button-rounded p-button-text p-button-plain h-2rem w-2rem p-0 inline-flex align-items-center justify-content-center"
            ...
            <Button v-tooltip.bottom="{ value: 'Copy Code', class: 'doc-section-code-tooltip' }" type="button"
                @click="copyCode"
                class="p-button-rounded p-button-text p-button-plain h-2rem w-2rem p-0 inline-flex align-items-center justify-content-center"
